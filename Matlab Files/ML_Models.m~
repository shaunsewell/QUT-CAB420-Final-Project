...
figure();
hold on;
scatter(alpha, lin_train, 50, 'filled', 'db');
scatter(alpha, lin_test, 50, 'filled', 'dr');
scatter(alpha, log_train, 50, 'filled', 'sb');
scatter(alpha, log_test, 50, 'filled', 'sr');
scatter(alpha, svm_train, 50, 'filled', 'hb');
scatter(alpha, svm_test, 50, 'filled', 'hr');
title('Decaying Average Error Rates');
xlabel('Alpha');
ylabel('Error');
...
for i=1:3
    figure();
    hold on;
    scatter(K, PCAC_Train_Mean(:,1,i,:), 50, 'filled','db');
    scatter(K, PCAC_Test_Mean(:,1,i,:), 50, 'filled','dr');
    scatter(K, PCAC_Train_Mean(:,2,i,:), 50, 'filled','sb');
    scatter(K, PCAC_Test_Mean(:,2,i,:), 50, 'filled','sr');
    scatter(K, PCAC_Train_Mean(:,3,i,:), 50, 'filled','hb');
    scatter(K, PCAC_Test_Mean(:,3,i,:), 50, 'filled','hr');
    title(strcat(int2str(pca(i)), ' Principle Components Error Rates'));
    xlabel('Clusters');
    ylabel('Error');
    legend('Linear Training', 'Linear Testing', 'Logistic Training', 'Logistic Testing', 'SVM Training', 'SVM Testing');
    hold off;
